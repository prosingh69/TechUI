...
const row1Techs = [
  ...
  },
  {
    name: 'Python',
    category: 'AI & Data',
    desc: 'FastAPI & machine learning APIs',
    icon: (
  ...
  },
  {
    name: 'Node.js',
    category: 'Backend',
    desc: 'High throughput event loops',
    icon: (
      <svg viewBox="0 0 256 256" className="w-7 h-7 sm:w-8 sm:h-8">
  ...
  },
  {
    name: 'AWS',
    category: 'Cloud',
    desc: 'Auto-scaling serverless cloud',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
        <path
...
const row2Techs = [
  {
    name: 'React 19',
    category: 'Frontend',
    desc: 'Declarative component architecture',
    icon: (
  ...
  },
  {
    name: 'Next.js 15',
    category: 'Frontend',
    desc: 'SSR & Edge rendering',
    icon: (
      <svg viewBox="0 0 180 180" className="w-7 h-7 sm:w-8 sm:h-8">
...
// Row 3: Has 6 icons! (Index 0 is in the notch right next to card)
const row3Techs = [
  {
    name: 'Gemini AI',
    category: 'AI Model',
  ...
  },
  {
    name: 'MongoDB',
    category: 'Database',
    desc: 'Flexible distributed documents',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
        <path
...
// Row 4: Has 6 icons! (Index 0 is in the notch)
const row4Techs = [
  {
    name: 'Apple iOS',
    category: 'Mobile',
    desc: 'Native Swift iOS ecosystem',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
        <path
  ...
  },
  {
    name: 'Android',
    category: 'Mobile',
    desc: 'Kotlin Android material app',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
        <path
  ...
  },
  {
    name: 'Shopify',
    category: 'Commerce',
    desc: 'Custom headless storefronts',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
        <path
  ...
  },
  {
    name: 'Stripe',
    category: 'Fintech',
    desc: 'Recurring billing & payments',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
  ...
  },
  {
    name: 'Figma',
    category: 'Design',
    desc: 'Design tokens & UX prototype',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
  ...
  },
  {
    name: 'GitHub',
    category: 'DevOps',
    desc: 'Continuous CI/CD deployment',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
        <path
          fillRule="evenodd"
...
const row5Techs = [
  ...
  },
  {
    name: 'FastAPI',
    category: 'Backend',
    desc: 'High performance async Python',
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 sm:w-8 sm:h-8">
...
const TechStack = () => {
  ...
  const renderCard = (tech) => (
    <motion.div
      ...
      className="bg-white rounded-2xl w-[60px] h-[60px] sm:w-[68px] sm:h-[68px] flex items-center justify-center shadow-md hover:shadow-xl transition-all duration-200 cursor-pointer group relative border border-slate-100 shrink-0"
    >
      ...
      {/* Tooltip */}
      <div className="absolute -top-9 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[11px] font-medium font-ibm py-1 px-2.5 rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 whitespace-nowrap shadow-lg z-30">
        {tech.name}
      </div>
    </motion.div>
  );
  ...
  return (
    <section className="w-full bg-[#1F40CB] text-white py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden font-ibm">
      {/* Centered Heading: Exact Structure from Reference Image */}
      <div className="max-w-4xl mx-auto text-center mb-10 sm:mb-14">
        <h2 className="text-3xl sm:text-4xl lg:text-[3.25rem] font-medium tracking-tight text-white leading-[1.14]">
          Create your perfect
          <br />
      ...
      <div className="max-w-6xl mx-auto flex justify-center">
        {/* ========================================================= */}
        {/* DESKTOP INTERLOCKING BENTO BOARD (Matches Image Layout)    */}
        {/* ========================================================= */}
        <div className="hidden lg:flex items-start gap-3.5 relative">
          
          {/* Left Column Stack: Stepped Highlight Card + Action CTA */}
          <div className="flex flex-col gap-3.5 w-[380px] shrink-0">
            {/* 1. Main Highlight Card (with Right Overhang Tab) */}
            <div className="relative bg-white rounded-3xl p-7 shadow-2xl z-10">
              {/* Overhang tab extending into Row 1 & 2 of the right grid */}
              <div 
                className="absolute top-0 -right-[80px] w-[80px] h-[148px] bg-white rounded-r-3xl z-10 pointer-events-none"
                style={{
                  clipPath: 'polygon(0 0, 100% 0, 100% 100%, 0 100%)'
                }}
              />

              {/* Square Badge Pill at Top Left */}
              <div className="w-12 h-12 rounded-xl bg-blue-50 text-[#1F40CB] flex items-center justify-center mb-7 shadow-sm">
                <LayoutGrid className="w-6 h-6 text-[#1F40CB]" />
              </div>

              {/* Text Description matching reference image structure */}
              <p className="text-[#0F172A] font-ibm text-[1.1rem] leading-[1.65] font-normal">
            ...
            {/* 2. Bottom Action Box (Directly below the Main Card) */}
            <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-lg flex items-center justify-between gap-4 z-10">
              <span className="text-[14px] font-medium text-[#1E293B] font-ibm leading-snug">
                See how Big Feathers connects with your favorite tools
              </span>
              <a
                href="#contact"
                className="shrink-0 inline-flex items-center gap-1.5 bg-[#1F40CB] hover:bg-[#1635AA] text-white font-semibold text-xs sm:text-sm px-4 py-2.5 rounded-xl transition-all shadow-sm hover:shadow-md"
              >
          ...
          {/* Right Column: Dark Navy Container with Interlocking Notched Grid */}
          <div className="relative bg-[#0B1C3D] border border-white/10 rounded-3xl p-3.5 shadow-2xl flex flex-col gap-3">
            {/* Row 1: 5 Icons (shifted right because tab covers col 1) */}
            <div className="flex items-center gap-3 pl-[80px]">
              {row1Techs.map(renderCard)}
            </div>

            {/* Row 2: 5 Icons (shifted right because tab covers col 1) */}
            <div className="flex items-center gap-3 pl-[80px]">
              {row2Techs.map(renderCard)}
            </div>

            {/* Row 3: 6 Icons (Full width into the notch!) */}
            <div className="flex items-center gap-3">
              {row3Techs.map(renderCard)}
            </div>

            {/* Row 4: 6 Icons (Full width into the notch!) */}
            <div className="flex items-center gap-3">
              {row4Techs.map(renderCard)}
            </div>

            {/* Row 5: 5 Icons (Aligned with bottom action box) */}
            <div className="flex items-center gap-3 pl-[80px]">
              {row5Techs.map(renderCard)}
            </div>

            {/* Tooltip / Status hint inside Navy container */}
            <div className="pt-2 px-1 text-xs text-cyan-200/75 flex items-center justify-between border-t border-white/5 font-ibm">
              <span>
                {hoveredTech ? (
                  <span className="text-white font-medium">
                    {hoveredTech.name} — <span className="text-cyan-200/90">{hoveredTech.desc}</span>
                  </span>
                ) : (
                  'Hover over any tool to inspect architecture integration'
                )}
              </span>
              <span className="text-[11px] text-white/40">26 Production Frameworks</span>
            </div>
          </div>
        ...
        {/* ========================================================= */}
        <div className="flex lg:hidden flex-col gap-6 w-full max-w-lg mx-auto">
          {/* Main Highlight Card */}
          <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl">
            <div className="w-12 h-12 rounded-xl bg-blue-50 text-[#1F40CB] flex items-center justify-center mb-5">
              <LayoutGrid className="w-6 h-6 text-[#1F40CB]" />
            </div>
            <p className="text-[#0F172A] font-ibm text-base sm:text-lg leading-relaxed font-normal">
              Connect your digital vision with{' '}
              <strong className="font-bold text-[#0F172A]">
                50+ battle-tested tools
              </strong>{' '}
          ...
          {/* Grid Container in Dark Navy */}
          <div className="bg-[#0B1C3D] border border-white/10 rounded-3xl p-5 shadow-xl">
            <div className="text-xs font-semibold text-cyan-300 uppercase tracking-wider mb-4">
              Integrated Tech Ecosystem
            </div>
            <div className="grid grid-cols-4 sm:grid-cols-5 gap-2.5 sm:gap-3 place-items-center">
              {[...row1Techs, ...row2Techs, ...row3Techs.slice(0, 4), ...row4Techs.slice(0, 4)].map(renderCard)}
            </div>
          </div>

          {/* Bottom Action Box */}
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-sm font-semibold text-[#1E293B] text-center sm:text-left">
              See how Big Feathers connects with your favorite tools
            </span>
            <a
              href="#contact"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 bg-[#1F40CB] hover:bg-[#1635AA] text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition-all shadow-sm"
            >
              <span>Explore Integrations</span>
