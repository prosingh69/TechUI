...
const GrowthThinking = () => {
  return (
    <div className="w-full bg-white dark:bg-[#030A1E] border-t border-gray-200/70 dark:border-white/10 shadow-[0_-4px_25px_rgba(0,0,0,0.06)] dark:shadow-[0_-4px_25px_rgba(0,0,0,0.3)] px-6 sm:px-8 lg:px-12 py-2 sm:py-2.5 transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 sm:gap-5 lg:gap-6">
          {articles.map((item, idx) => (
            <a
              key={item.id}
              href={item.link}
              className={`group flex items-start gap-3 sm:gap-3.5 p-0.5 sm:p-1 rounded-xl hover:bg-gray-50/80 dark:hover:bg-white/[0.05] transition-all duration-200 ${idx === 0 ? '-ml-0.5 sm:-ml-1' : ''}`}
            >
              {/* Thumbnail Box: Enlarged to prominent large format */}
              <div className="relative w-16 h-16 sm:w-[4.75rem] sm:h-[4.75rem] lg:w-[5.25rem] lg:h-[5.25rem] rounded-xl overflow-hidden bg-blue-950 shrink-0 shadow-sm border border-black/5 dark:border-white/10">
                <img
                  src={item.image}
                  alt={item.title}
              ...
              <div className="flex flex-col justify-between flex-1 min-w-0 h-16 sm:h-[4.75rem] lg:h-[5.25rem] py-0.5">
                <p className="text-[13px] sm:text-[13.5px] lg:text-[14.5px] font-medium text-[#1A1D2E] dark:text-white group-hover:text-blue-600 dark:group-hover:text-[#00A2ED] transition-colors line-clamp-2 leading-[1.34] font-ibm">
                  {item.title}
                </p>
                ...
                <div className="flex items-center justify-between mt-auto opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <span className="text-[9px] sm:text-[9.5px] font-bold text-blue-700 dark:text-[#00A2ED] bg-blue-50 dark:bg-white/10 px-1.5 py-0.5 rounded tracking-wider uppercase">
                    {item.category}
                  </span>
                  <svg
